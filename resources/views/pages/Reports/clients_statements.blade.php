...
<head>
    <title>
        {{ trans('page-contract.contract.titles.clientsstatmentss') }}
    </title>
...
<table  style="width:100%">
                ...
                <div class="row" >
                    <div class="col-md-4 text-center" style="text-align: center;background-color: #005490;color:white">
                        <div  style="text-align: center;padding: 45px 0;">
                            <h4>{{ trans('page-contract.contract.titles.clientlabel') }}</h4>
                            <h2> {{ $clientname[0]->pname }} </h2>
                            <h4> {{ trans('page-contract.contract.titles.billscount') }} : {{$count}} </h4>
                            @if($filter == 1)
                                <h4> {{ trans('page-contract.contract.titles.followbyname') }} : {{$partname}} </h4>
                            @elseif($filter == 2)
                                <h4> {{ trans('page-contract.contract.titles.brokerbyname') }} : {{$partname}} </h4>
                            @endif
                        </div>
                    </div>
                    <div class="col-md-4 text-center">
                        <div  style="text-align: center;padding: 40px 0;">
                            <h2> {{ trans('page-contract.contract.titles.clientsstatments') }} </h2>
                            <h2> {{ trans('page-contract.contract.titles.clientsstatments2') }} </h2>
                            <h2> ( {{ $billoption }} ) </h2>
        ...
        <tr style="background-color: #005490;color:#ffffff">
                    ...
                    <th >
                        <h6>{{ trans('page-contract.contract.tables.broker') }}</h6>
                    </th>
                    <th >
                        <h6>{{ trans('page-contract.contract.tables.followby') }}</h6>
                    </th>
                    <th >
        ...
        @foreach($contractlist as $colist)
            <tr>
                ...
                @endif
                @if($colist -> brokername == 'NONE')
                    <td ><p class="font-weight-bold">{{ trans('page-contract.contract.tables.nocar') }}</p></td>
                @else
                    <td ><p class="font-weight-bold">{{$colist -> brokername}}</p></td>
                @endif
                @if($colist -> followname == 'NONE')
                    <td ><p class="font-weight-bold">{{ trans('page-contract.contract.tables.nocar') }}</p></td>
                @else
                    <td ><p class="font-weight-bold">{{$colist -> followname}}</p></td>
                @endif
