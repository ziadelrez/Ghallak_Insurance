   ...
   <div class="wrapper">
    ...
    <header class="mb-3"  style="right: -200px; margin-right: 300px;" >
        <button class="burger-btn d-block d-xl-none"  id="sidebutton" >
            ...
            ?>
            <div class="page-heading" style="right: -200px; margin-right: 300px;">
                <h3>  دفعات الغراج : <?php echo $garage ?></h3>
            </div>
            <?php
            ...
            $q = "SELECT * FROM accident_cost WHERE garage_id = '" . $row3['garage_id'] . "'";
            ...
            <div class="page-content" style="left: -300px; margin-right: 300px; ">
                <div class="card-body">
                    <div class="container-table100">
                        <div class="wrap-table100">
                            <table class="table table-striped table-hover">
                                ...
                                <?php
                                mysqli_data_seek($resultat5,0);
                                $somme=0;
                                
                                for ($i = 0; $i < mysqli_num_rows($resultat5); $i++) {
                                    $row = mysqli_fetch_array($resultat5);
                                    global $row;
                                    if($row['lebanese_amount'] == null){
                                        $lb = 0;
                                    }else $lb = $row['lebanese_amount'];
                                    $somme+=$row['required_amount'];
                                    echo "<tr class='row2' onclick='garage_info(" . $row['garage_expenses_id'] . ")'>";
                                    echo "<td><input type='checkbox' onclick='GetSelected(event, " . $row['garage_expenses_id'] . ", " . $row['required_amount'] . ", " . $row['lebanese_amount'] . ", \"" . $row['accident_id'] . "\")'></td>";
                    ...
                    <div class="buttons">
                        <button type="submit" name="submit" onclick="wasel3()" formtarget="_blank" class="btn btn-primary"> الإيصال</button>
                    </div>
                </div>
            </div>
            <br>
            ...
            <script>
                ...
                var ar = new Array();
                var arr = new Array();
                function GetSelected(event, id, amount1, lebanese1, accident) {
                    //Reference the Table.
                ...
                var cont = new Set(arr);
                    var garage = <?php echo json_encode($garage); ?>;
                    var location = "garage_receipt.php?garage=" + garage + "&somme=" + amount + "&accidents=" + [...cont].join(" - ") + "&lebanese=" + lebanese;
            ...
            <div class="page-content" style="left: -300px; margin-right: 300px; ">
                <div class="card-body">
                    <div class="container-table100">
                        <div class="wrap-table100">
                            <table class="table table-striped table-hover">
                                ...
                                for ($i = 0; $i < mysqli_num_rows($r); $i++) {
                                    $roww = mysqli_fetch_array($r);
                                    $sql4="SELECT * FROM accident WHERE accident_id='".$roww['accident_id']."'";
                                    $res4=mysqli_query($con, $sql4);
                                    $row4=mysqli_fetch_array($res4);
                                    $sql5="SELECT * FROM person WHERE person_id = '".$row4['person_id']."'";
                    ...
                    <script type="text/javascript">
                        function garage_info(id) {
                            location.href = "edit_garage_expenses.php?garage_expenses_id=" + id;
                        };
                    </script>
