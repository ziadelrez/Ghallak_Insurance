...
$agent_expenses_id = $_GET['agent_expenses_id'];
$sql = "SELECT * FROM agent_expenses WHERE agent_expenses_id = '$agent_expenses_id'";
$res = mysqli_query($con, $sql);
$row = mysqli_fetch_array($res);
$sql2 = "SELECT * FROM person WHERE person_id = '" . $row['agent_id'] . "'";
...
if(!isset($_GET['contract_id'])){
    ...
    $sql7 = "SELECT * FROM agent_expenses WHERE agent_id = '" . $row['agent_id'] . "' AND contract_id = '" . $row['contract_id'] . "'";
    $res7 = mysqli_query($con, $sql7);
    for ($i = 0; $i < mysqli_num_rows($res7); $i++) {
        $row7 = mysqli_fetch_array($res7);
        $req2 += $row7['required_amount'];
    }
    $rest =  $row6['agent_portion'] - $req2 + $row['required_amount'];
}
else{
    $contract = $_GET['contract_id'];
    if ($contract != $row['contract_id']) {
        $sql6 = "SELECT * FROM contract WHERE contract_id = '$contract'";
        $res6 = mysqli_query($con, $sql6);
        $row6 = mysqli_fetch_array($res6);
        $sql7 = "SELECT * FROM agent_expenses WHERE agent_id = '" . $row['agent_id'] . "' AND contract_id = '$contract'";
        $res7 = mysqli_query($con, $sql7);
        for ($i = 0; $i < mysqli_num_rows($res7); $i++) {
            $row7 = mysqli_fetch_array($res7);
            $req2 += $row7['required_amount'];
        }
        $rest =  $row6['agent_portion'] - $req2;
    } else header('Location: edit_agent_expenses.php?agent_expenses_id=' . $agent_expenses_id);
...
<body>
   <div class="wrapper">
    ...
    <header class="mb-3"  style="right: -200px; margin-right: 300px;" >
        <button class="burger-btn d-block d-xl-none"  id="sidebutton" >
            ...
            <div class="page-content" style="left: -300px; margin-right: 300px; ">
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-8 col-md-12">
                            <form action="edit_agent_expenses_inter.php?agent_expenses_id=<?php echo $agent_expenses_id; ?>" method="post">
                                <div class="form-group">
                                    <label for="agent">العميل*</label>
                                    <input type="text" class="form-control" id="agent" name="agent" value="<?php echo $row2['person_name']; ?>" readonly>
                                    <div class="list-group list-group-item-action" id="agent_content"></div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="contract" class="btn btn-light dropdown-toggle dropdown-toggle-split" onchange="refresh()" id="contract" required>
                                        ...
                                        <?php
                                        $sql4 = "SELECT * FROM contract WHERE agent = '" . $row['agent_id'] . "'";
                                ...
                                <script>
                                    function refresh() {
                                        ...
                                        var id = <?php echo json_decode($agent_expenses_id); ?>;
                                        var select = contract.value;
                                        location.href = "edit_agent_expenses.php?agent_expenses_id=" + id + "&contract_id=" + select;
                                    }
                                </script>
                                <div class="form-group">
                ...
                <div class="buttons">
                    ...
                    <a href="agents_expenses.php" name="back" class="btn btn-danger"> رجوع </a>
                    <a href="delete_agent_expenses.php?agent_expenses_id=<?php echo $agent_expenses_id; ?>" class="btn btn-danger" id="delete_btn" onclick="return confirm('هل أنت متأكد ؟')">حذف الدفعة </a>
                </div>
                </form>
        ...
        <script src="assets/js/search7.js"></script>
        <script src="assets/js/search8.js"></script> 
